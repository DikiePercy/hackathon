...
- Пользователь может прикрепить фото в предложении (только изображение, размер строго меньше 2 МБ).
- Пользователь может отправить не только новую запись, но и изменение существующей (режим `update` + `target_person_id`).
